...
const MPPT_STAGE_COLOR = { 'Bulk': 'var(--warning, var(--warning, #ffd740))', 'Absorción': 'var(--accent, var(--accent, #1a6fff))', 'Float': 'var(--online, var(--online, #00e676))', 'Equalización': '#ff9800' }

const DOOR_LABELS = ['Principal', 'Gabinete Solar', 'Compartimento A', 'Compartimento B']

function MetricChip({ label, value, unit, color = 'var(--text-primary, var(--text-primary, #e8f0fe))', mono = true, small = false }) {
  return (
    <div style={{ background: 'var(--bg-input, var(--bg-input, #111f35))', borderRadius: 8, padding: small ? '8px 12px' : '10px 14px', border: '1px solid #1a3050' }}>
      <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, color: 'var(--text-muted, var(--text-muted, #3d5a80))', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 }}>
        {label}
      </div>
      <div style={{ fontFamily: mono ? "'DM Mono',monospace" : "'Syne',sans-serif", fontSize: small ? 14 : 18, fontWeight: 700, color }}>
        {value ?? '—'}<span style={{ fontSize: 11, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', marginLeft: 3 }}>{unit}</span>
      </div>
    </div>
  )
}

function SectionTitle({ children, color = 'var(--accent, var(--accent, #5a9fff))' }) {
  return (
...
const CUSTOM_TOOLTIP = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: 'var(--bg-input, var(--bg-input, #111f35))', border: '1px solid #1a3050', borderRadius: 8, padding: '8px 12px', fontSize: 11 }}>
      <div style={{ color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', marginBottom: 4, fontFamily: "'DM Mono',monospace" }}>{label}</div>
      {payload.map(p => (
...
export default function DeviceDetail() {
  ...
  if (!device) return (
    <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted, var(--text-muted, #3d5a80))' }}>
      Cargando parada...
    </div>
  )
  ...
  const mpptStageColor = MPPT_STAGE_COLOR[mpptStage] || 'var(--text-secondary, var(--text-secondary, #6b8ab0))'
  const rssi = t.rssi ?? '--'
  const rsrp = t.rsrp ?? '--'
  const rsrq = t.rsrq ?? '--'
  const snr = t.snr ?? '--'
  ...
  const socColor = typeof soc === 'number'
    ? (soc > 50 ? 'var(--online, var(--online, #00e676))' : soc > 20 ? 'var(--warning, var(--warning, #ffd740))' : 'var(--offline, var(--offline, #ff5252))')
    : 'var(--text-secondary, var(--text-secondary, #6b8ab0))'

  const rssiColor = typeof rssi === 'number'
    ? (rssi > -80 ? 'var(--online, var(--online, #00e676))' : rssi > -95 ? 'var(--warning, var(--warning, #ffd740))' : 'var(--offline, var(--offline, #ff5252))')
    : 'var(--text-secondary, var(--text-secondary, #6b8ab0))'
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: isMobile ? '10px 12px' : '12px 24px', borderBottom: '1px solid #1a3050', display: 'flex', alignItems: 'center', gap: isMobile ? 10 : 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {t.buffered_positions > 0 && (
            <span style={{ fontSize: 10, color: 'var(--warning, var(--warning, #ffd740))', background: 'rgba(255,215,64,0.1)', border: '1px solid rgba(255,215,64,0.3)', borderRadius: 20, padding: '2px 8px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 0, borderBottom: '1px solid #1a3050' }}>
        {[
          ...
          { l: 'Voltaje', v: typeof voltage === 'number' ? voltage.toFixed(2) : voltage, u: 'V', c: 'var(--accent, var(--accent, #5a9fff))' },
          { l: 'P. Carga', v: typeof chargeW === 'number' ? chargeW.toFixed(0) : chargeW, u: 'W', c: 'var(--online, var(--online, #00e676))' },
          { l: 'P. Descarga', v: typeof dischargeW === 'number' ? dischargeW.toFixed(0) : dischargeW, u: 'W', c: 'var(--offline, var(--offline, #ff5252))' },
          { l: 'Panel', v: typeof panelW === 'number' ? panelW.toFixed(0) : panelW, u: 'W', c: 'var(--warning, var(--warning, #ffd740))' },
          ...
          <div key={i} style={{
            ...
            <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, color: 'var(--text-muted, var(--text-muted, #3d5a80))', letterSpacing: 1, textTransform: 'uppercase' }}>{m.l}</div>
            <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 16, fontWeight: 700, color: m.c, marginTop: 2 }}>
              {m.v}<span style={{ fontSize: 10, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', marginLeft: 2 }}>{m.u}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', borderBottom: '1px solid #1a3050', background: 'var(--bg-card, var(--bg-card, #0c1829))' }}>
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
            padding: '10px 20px', background: 'none', border: 'none',
            borderBottom: activeTab === tab.id ? '2px solid #1a6fff' : '2px solid transparent',
            color: activeTab === tab.id ? 'var(--accent, var(--accent, #5a9fff))' : 'var(--text-secondary, var(--text-secondary, #6b8ab0))',
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: isMobile ? '12px' : '20px 24px' }}>
        ...
        {activeTab === 'battery' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: '1px solid #1a3050', borderRadius: 12, padding: isMobile ? '14px 14px' : '20px 24px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: "'DM Mono',monospace", fontSize: 9, color: 'var(--text-muted, var(--text-muted, #3d5a80))' }}>
            ...
            <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: '1px solid #1a3050', borderRadius: 12, padding: '16px 20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
                {cells.map((cv, i) => {
                  ...
                  const cc = cv > 3.3 ? 'var(--online, var(--online, #00e676))' : cv > 3.0 ? 'var(--warning, var(--warning, #ffd740))' : 'var(--offline, var(--offline, #ff5252))'
                  return (
                    <div key={i} style={{ background: 'var(--bg-input, var(--bg-input, #111f35))', borderRadius: 8, padding: '12px', border: '1px solid #1a3050', textAlign: 'center' }}>
                      <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, color: 'var(--text-muted, var(--text-muted, #3d5a80))', marginBottom: 8 }}>CELDA {i+1}</div>
                      <div style={{ height: 60, background: 'var(--border, var(--border, #1a3050))', borderRadius: 4, overflow: 'hidden', position: 'relative', marginBottom: 8 }}>
                      ...
                      <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 14, fontWeight: 700, color: cc }}>
                        {cv ? cv.toFixed(3) : '—'}<span style={{ fontSize: 9, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', marginLeft: 2 }}>V</span>
                      </div>
                    </div>
                  )
                })}
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, fontFamily: "'DM Mono',monospace", fontSize: 9, color: 'var(--text-muted, var(--text-muted, #3d5a80))' }}>
              ...
              <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: '1px solid #1a3050', borderRadius: 12, padding: '16px 20px' }}>
                ...
                <ResponsiveContainer width="100%" height={160}>
                  <AreaChart data={socHistory}>
                    ...
                    <XAxis dataKey="h" tick={{ fill: 'var(--text-muted, var(--text-muted, #3d5a80))', fontSize: 9 }} />
                    <YAxis domain={[0, 100]} tick={{ fill: 'var(--text-muted, var(--text-muted, #3d5a80))', fontSize: 9 }} />
                    <ReferenceLine y={20} stroke="#ff5252" strokeDasharray="4 4" label={{ value: '20% mín', fill: 'var(--offline, var(--offline, #ff5252))', fontSize: 8 }} />
        ...
        {activeTab === 'solar' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {/* Estado MPPT */}
            <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: `1px solid ${mpptStageColor}40`, borderRadius: 12, padding: '16px 20px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', lineHeight: 1.8 }}>
              ...
              <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: '1px solid #1a3050', borderRadius: 12, padding: '16px 20px' }}>
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={solarHistory}>
                    <XAxis dataKey="h" tick={{ fill: 'var(--text-muted, var(--text-muted, #3d5a80))', fontSize: 9 }} />
                    <YAxis domain={[0, PANEL.maxW]} tick={{ fill: 'var(--text-muted, var(--text-muted, #3d5a80))', fontSize: 9 }} />
                    <Tooltip content={<CUSTOM_TOOLTIP />} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: '1px solid #1a3050', borderRadius: 12, padding: '14px 18px' }}>
                ...
                {[
                  ...
                  <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid rgba(26,48,80,0.5)' }}>
                    <span style={{ fontSize: 11, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))' }}>{l}</span>
                    <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 11, color: 'var(--text-primary, var(--text-primary, #e8f0fe))' }}>{v}</span>
                  </div>
                ))}
              </div>
              <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: '1px solid #1a3050', borderRadius: 12, padding: '14px 18px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: '1px solid #1a3050', borderRadius: 12, padding: isMobile ? '14px 14px' : '20px 24px', textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 48, fontWeight: 800, color: '#ff9800' }}>
                  ...
                  <span style={{ fontSize: 20, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))' }}>°C</span>
                </div>
                <div style={{ fontSize: 11, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', marginTop: 8 }}>
              ...
              <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: '1px solid #1a3050', borderRadius: 12, padding: isMobile ? '14px 14px' : '20px 24px', textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 48, fontWeight: 800, color: 'var(--accent2, var(--accent2, #00d4ff))' }}>
                  {typeof humidity === 'number' ? humidity.toFixed(1) : humidity}
                  <span style={{ fontSize: 20, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))' }}>%</span>
                </div>
                <div style={{ fontSize: 11, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', marginTop: 8 }}>
            ...
            <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: '1px solid #1a3050', borderRadius: 12, padding: '16px 20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
                {DOOR_LABELS.map((label, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      background: 'var(--bg-input, var(--bg-input, #111f35))', borderRadius: 10, padding: '16px',
                      ...
                      <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', marginBottom: 6 }}>
                        Puerta {i+1}
                      </div>
                      ...
                      <div style={{
                        ...
                        color: open ? 'var(--offline, var(--offline, #ff5252))' : 'var(--online, var(--online, #00e676))',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: '1px solid #1a3050', borderRadius: 12, padding: '16px 20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 12 }}>
                ...
                <MetricChip label="RSRP" value={rsrp} unit="dBm" color={typeof rsrp === 'number' ? (rsrp > -90 ? 'var(--online, var(--online, #00e676))' : rsrp > -105 ? 'var(--warning, var(--warning, #ffd740))' : 'var(--offline, var(--offline, #ff5252))') : 'var(--text-secondary, var(--text-secondary, #6b8ab0))'} />
                <MetricChip label="RSRQ" value={rsrq} unit="dB" color="#00d4ff" />
                <MetricChip label="SNR" value={snr} unit="dB" color="#5a9fff" />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
                <div style={{ background: 'var(--bg-input, var(--bg-input, #111f35))', borderRadius: 8, padding: '10px 14px', border: '1px solid #1a3050' }}>
                  <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, color: 'var(--text-muted, var(--text-muted, #3d5a80))', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 }}>BANDA LTE</div>
                  <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 16, fontWeight: 800, color: 'var(--accent2, var(--accent2, #00d4ff))' }}>{lteBand}</div>
                  <div style={{ fontSize: 10, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', marginTop: 2 }}>LTE Cat4 FDD</div>
                </div>
                <div style={{ background: 'var(--bg-input, var(--bg-input, #111f35))', borderRadius: 8, padding: '10px 14px', border: '1px solid #1a3050' }}>
                  <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, color: 'var(--text-muted, var(--text-muted, #3d5a80))', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 }}>PRECISIÓN GPS</div>
                  <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 16, fontWeight: 700, color: 'var(--online, var(--online, #00e676))' }}>&lt;2.0</div>
                  <div style={{ fontSize: 10, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', marginTop: 2 }}>metros CEP (u-blox)</div>
                </div>
                <div style={{ background: 'var(--bg-input, var(--bg-input, #111f35))', borderRadius: 8, padding: '10px 14px', border: '1px solid #1a3050' }}>
                  <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, color: 'var(--text-muted, var(--text-muted, #3d5a80))', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 }}>BUFFER LOCAL</div>
                  <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 16, fontWeight: 700, color: 'var(--warning, var(--warning, #ffd740))' }}>
                    {t.buffered_positions ?? 0}
                  </div>
                  <div style={{ fontSize: 10, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', marginTop: 2 }}>/ 30.000 posiciones</div>
                </div>
                <div style={{ background: 'var(--bg-input, var(--bg-input, #111f35))', borderRadius: 8, padding: '10px 14px', border: '1px solid #1a3050' }}>
                  <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, color: 'var(--text-muted, var(--text-muted, #3d5a80))', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 }}>UPTIME</div>
                  <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 13, fontWeight: 700, color: 'var(--text-primary, var(--text-primary, #e8f0fe))' }}>{fmtUptime(uptime)}</div>
                  <div style={{ fontSize: 10, color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', marginTop: 2 }}>Autonomía batt.: {GV310.autonomyH}h</div>
                </div>
              </div>
            </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {['B1','B2','B3','B4','B5','B7','B8','B28'].map(b => (
                  <span key={b} style={{
                    ...
                    background: lteBand === b ? 'rgba(0,212,255,0.2)' : 'var(--bg-hover, rgba(26,48,80,0.5))',
                    color: lteBand === b ? 'var(--accent2, var(--accent2, #00d4ff))' : 'var(--text-secondary, var(--text-secondary, #6b8ab0))',
                    border: `1px solid ${lteBand === b ? 'rgba(0,212,255,0.4)' : 'var(--border, var(--border, #1a3050))'}`,
                ...
                {['B1','B2','B5','B8'].map(b => (
                  <span key={'w'+b} style={{ padding: '3px 10px', borderRadius: 20, fontSize: 10, fontFamily: "'DM Mono',monospace", background: 'rgba(26,48,80,0.3)', color: 'var(--text-secondary, var(--text-secondary, #6b8ab0))', border: '1px solid #1a3050' }}>
                    WCDMA {b}
                  </span>
                ))}
              </div>
              ...
              <div style={{ background: 'var(--bg-card, var(--bg-card, #0c1829))', border: '1px solid #1a3050', borderRadius: 12, padding: '16px 20px' }}>
                ...
                <ResponsiveContainer width="100%" height={160}>
                  <LineChart data={signalHistory}>
                    <XAxis dataKey="h" tick={{ fill: 'var(--text-muted, var(--text-muted, #3d5a80))', fontSize: 9 }} />
                    <YAxis domain={[-120, -50]} tick={{ fill: 'var(--text-muted, var(--text-muted, #3d5a80))', fontSize: 9 }} />
                    <ReferenceLine y={-100} stroke="#ff5252" strokeDasharray="4 4" label={{ value: 'Señal débil', fill: 'var(--offline, var(--offline, #ff5252))', fontSize: 8 }} />
                    <Tooltip content={<CUSTOM_TOOLTIP />} />
        ...
        {activeTab === 'events' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, color: 'var(--text-muted, var(--text-muted, #3d5a80))', letterSpacing: 1, textTransform: 'uppercase' }}>
              Últimos 20 eventos — {device.name}
            </div>
            {recentEvents.length === 0 && (
              <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted, var(--text-muted, #3d5a80))' }}>Sin eventos registrados</div>
            )}
            {recentEvents.map(ev => {
              const sev = { info: 'var(--accent, var(--accent, #5a9fff))', warning: 'var(--warning, var(--warning, #ffd740))', critical: 'var(--offline, var(--offline, #ff5252))' }
              const icon = { info: 'ℹ', warning: '⚠', critical: '🚨' }
              return (
                <div key={ev.id} style={{
                  background: 'var(--bg-card, var(--bg-card, #0c1829))', borderRadius: 10, padding: '12px 16px',
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      ...
                      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, color: 'var(--text-muted, var(--text-muted, #3d5a80))' }}>
                        {ev.ts ? new Date(ev.ts.endsWith('Z') ? ev.ts : ev.ts + 'Z').toLocaleString('es-EC', { timeZone: 'America/Guayaquil' }) : '--'}
                      </span>
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--text-primary, var(--text-primary, #e8f0fe))' }}>{ev.message}</div>
                    {ev.resolved && <div style={{ marginTop: 4, fontSize: 10, color: 'var(--online, var(--online, #00e676))' }}>✓ Resuelto</div>}
                  </div>
                </div>
              )
            })}
