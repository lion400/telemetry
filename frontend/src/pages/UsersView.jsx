...
const ROLES = {
  ...
  supervisor: { color: '#ffd740', label: 'Supervisor', icon: '🔭' },
  operador:   { color: '#5a9fff', label: 'Operador',   icon: '🛠' },
}
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          background: 'linear-gradient(135deg, #0c1829, #111f35)', flexShrink: 0,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button onClick={onClose} style={{
              ...
              color: '#6b8ab0', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>✕</button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
            {[
              { label: 'Paradas asignadas', value: assignedDevices.length, total: devices.length, color: '#1a6fff' },
              { label: 'Tipos de reporte',  value: assignedReports.length, total: REPORT_TYPES.length, color: '#00e676' },
            ].map(s => (
              <div key={s.label} style={{
                flex: 1, background: 'rgba(26,48,80,0.4)', borderRadius: 8,
                padding: '8px 14px', border: '1px solid #1a3050',
              }}>
                <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 22, fontWeight: 800, color: s.color }}>
                  {loading ? '…' : s.value}
                  <span style={{ fontSize: 12, color: '#3d5a80', fontWeight: 400 }}> / {s.total}</span>
                </div>
                <div style={{ fontSize: 10, color: '#6b8ab0', fontFamily: "'DM Mono',monospace", marginTop: 2 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid #1a3050', flexShrink: 0 }}>
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              ...
              background: tab === t.id ? 'rgba(26,111,255,0.08)' : 'transparent',
              border: 'none', borderBottom: `2px solid ${tab === t.id ? '#1a6fff' : 'transparent'}`,
              color: tab === t.id ? '#5a9fff' : '#6b8ab0',
              fontSize: 13, fontWeight: tab === t.id ? 700 : 400, transition: 'all 0.15s',
            }}>
              {t.label}
              {t.count > 0 && (
                <span style={{
                  marginLeft: 6, background: tab === t.id ? '#1a6fff' : '#1a3050',
                  color: tab === t.id ? '#fff' : '#6b8ab0',
        ...
        <div style={{ flex: 1, overflow: 'auto', minHeight: 0 }}>
          {loading ? (
            <div style={{ padding: 40, textAlign: 'center', color: '#3d5a80', fontSize: 13 }}>
            ...
            <div style={{ padding: '14px 20px' }}>
              <div style={{ display: 'flex', gap: 8, marginBottom: 14, alignItems: 'center' }}>
                <input
                  ...
                  style={{
                    flex: 1, background: '#111f35', border: '1px solid #1a3050',
                    borderRadius: 8, padding: '7px 12px', color: '#e8f0fe', fontSize: 12, outline: 'none',
                  }}
                />
                <button onClick={() => setAssignedDevices(devices.map(d => d.device_id))} style={{
                  ...
                  borderRadius: 7, padding: '6px 12px', color: '#5a9fff', cursor: 'pointer', fontSize: 11, whiteSpace: 'nowrap',
                }}>✓ Todas</button>
                <button onClick={() => setAssignedDevices([])} style={{
                  background: 'none', border: '1px solid #1a3050',
                  borderRadius: 7, padding: '6px 12px', color: '#6b8ab0', cursor: 'pointer', fontSize: 11, whiteSpace: 'nowrap',
                }}>✕ Ninguna</button>
              </div>
              ...
              {Object.entries(grouped).map(([group, devs]) => (
                <div key={group} style={{ marginBottom: 16 }}>
                  <div style={{
                    fontSize: 9, color: '#3d5a80', fontFamily: "'DM Mono',monospace",
                    letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8,
                    display: 'flex', alignItems: 'center', gap: 8,
                  }}>
                    <span>{group}</span>
                    <span style={{ background: '#1a3050', borderRadius: 8, padding: '1px 6px' }}>{devs.length}</span>
                    ...
                    {devs.map(d => {
                      ...
                      return (
                        <div key={d.device_id} onClick={() => toggleDevice(d.device_id)} style={{
                          ...
                          background: checked ? 'rgba(26,111,255,0.12)' : 'rgba(26,48,80,0.25)',
                          border: `1px solid ${checked ? 'rgba(26,111,255,0.4)' : 'rgba(26,48,80,0.5)'}`,
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{
                              fontSize: 12, fontWeight: 600, color: checked ? '#e8f0fe' : '#b0c4de',
                              overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                            }}>{d.name}</div>
                            <div style={{ fontSize: 9, color: '#3d5a80', marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              {d.address}
                            </div>
                          </div>
                          <div style={{
              ...
              {filtered.length === 0 && (
                <div style={{ textAlign: 'center', color: '#3d5a80', padding: 30, fontSize: 13 }}>
            ...
            <div style={{ padding: '20px' }}>
              <div style={{ fontSize: 12, color: '#6b8ab0', marginBottom: 16 }}>
                ...
                {REPORT_TYPES.map(rt => {
                  ...
                  return (
                    <div key={rt.id} onClick={() => toggleReport(rt.id)} style={{
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: checked ? '#e8f0fe' : '#b0c4de' }}>
                          {rt.label}
                        </div>
                        <div style={{ fontSize: 11, color: '#6b8ab0', marginTop: 3 }}>{rt.desc}</div>
                      </div>
                      {checked && (
                        <span style={{
                          fontSize: 9, fontFamily: "'DM Mono',monospace", color: '#1a6fff',
        ...
        <div style={{
          ...
          background: '#0a1628', flexShrink: 0, display: 'flex', gap: 10,
        }}>
          <button onClick={onClose} style={{
            background: 'none', border: '1px solid #1a3050', borderRadius: 8,
            padding: '10px 20px', cursor: 'pointer', color: '#6b8ab0', fontSize: 13,
          }}>Cancelar</button>
          <button onClick={save} disabled={saving} style={{
            flex: 1,
            background: saving ? '#1a3050' : 'linear-gradient(135deg, rgba(26,111,255,0.3), rgba(26,111,255,0.15))',
            border: `1px solid ${saving ? '#1a3050' : 'rgba(26,111,255,0.5)'}`,
            borderRadius: 8, padding: '10px', cursor: saving ? 'default' : 'pointer',
            color: saving ? '#3d5a80' : '#5a9fff', fontSize: 13, fontWeight: 700,
          }}>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {/* Formulario */}
        {showForm && (
          <div style={{ background: '#0c1829', border: '1px solid #1a3050', borderRadius: 12, padding: 20, maxWidth: 480 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tr style={{ background: '#060d1a' }}>
                {['Usuario', 'Rol', 'Estado', 'Último acceso', 'Acciones'].map(h => (
                  <th key={h} style={{
                    ...
                    color: '#3d5a80', textTransform: 'uppercase', fontWeight: 400,
                  }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {users.map((u, i) => {
                ...
                return (
                  <tr key={u.id}
                    style={{ borderBottom: '1px solid rgba(26,48,80,0.5)', background: i % 2 === 0 ? 'transparent' : 'rgba(26,48,80,0.1)' }}
                    onMouseEnter={e => e.currentTarget.style.background = 'rgba(26,111,255,0.05)'}
                    onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? 'transparent' : 'rgba(26,48,80,0.1)'}
                  >
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      {me?.role === 'gerente' && u.id !== me?.id ? (
                        <select value={u.role} onChange={e => changeRole(u.id, e.target.value)} style={{
                          background: '#111f35', border: `1px solid ${rol.color}44`,
                    ...
                    <td style={{ padding: '12px 16px', fontFamily: "'DM Mono',monospace", fontSize: 10, color: '#6b8ab0' }}>
                      {u.last_login
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button onClick={() => setModalUser(u)} style={{
                          ...
                          color: '#5a9fff', fontSize: 11, fontWeight: 600,
                        }}>⚙ Asignar</button>
                        {me?.role === 'gerente' && u.id !== me?.id && (
        ...
        {log.length > 0 && (
          <div style={{ background: '#0c1829', border: '1px solid #1a3050', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #1a3050', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, color: '#3d5a80', background: '#111f35', padding: '2px 8px', borderRadius: 8 }}>
            ...
            <div style={{ maxHeight: 240, overflow: 'auto' }}>
              {log.slice(0, 40).map((l, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ color: '#6b8ab0', fontSize: 10, fontFamily: "'DM Mono',monospace", overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
