...
export default function AppShell() {
  ...
  const [collapsed, setCollapsed] = useState(true) // ahora empieza colapsado
  const [hovered, setHovered] = useState(false)    // para hover temporal
  ...
  // En desktop: ancho depende de collapsed y hover
  const sidebarWidth = isMobile ? 0 : (!collapsed ? 220 : (hovered ? 220 : 56))
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 12 }}>
            {!isMobile && (
              <button
                ...
                style={{ background: 'none', border: 'none', color: 'var(--text-secondary, #6b8ab0)', cursor: 'pointer', fontSize: 20, padding: '4px 6px', borderRadius: 6, width: 32 }}
            ...
            <div style={{
              ...
              opacity: (!isMobile && collapsed && !hovered) ? 0 : 1, // ocultar texto cuando colapsado y no hover
        ...
        {!isMobile && (
          ...
          >
            {filteredNav.map(n => {
              ...
              // En modo colapsado sin hover, solo mostramos íconos
              const showLabel = sidebarWidth > 56 // si está expandido (por click o hover)
              return (
                <div
                  key={n.id}
                  onClick={() => navigate(n.id)}
                  title={!showLabel ? n.label : ''}
                  style={{
                ...
                >
                  ...
                  {showLabel && <span style={{ overflow: 'hidden' }}>{n.label}</span>}
                  {n.badge === 'unread' && unreadEvents > 0 && showLabel && (
                    <span style={{
